import { useState, useEffect, useMemo } from "react";
import { useDispatch, useSelector } from "react-redux";
import { Link } from "react-router-dom";
import { IoClose, IoChevronBack } from "react-icons/io5";
import { FaChevronRight } from "react-icons/fa";
import {
  followersSelector,
  getFollowRequestsAsync,
  getActivityAsync,
  acceptFollowRequestAsync,
  rejectFollowRequestAsync,
  toggleFollowAsync,
  getFollowingAsync,
} from "../../redux/slices/followersSlice";
import { usersSelector } from "../../redux/slices/usersSlice";
import { formatTimeAgo, isVideoMedia } from "../../utils";
import Avatar from "../common/Avatar";
import PostDetailsModal from "../post/PostDetailsModal";

export function NotificationsDrawer({ isOpen, onClose }) {
  const dispatch = useDispatch();
  const { requests = [], activity = [], following = [], loading } = useSelector(
    followersSelector
  );
  const { userId: currentUserId, signedUser } = useSelector(usersSelector);
  const [viewingAllRequests, setViewingAllRequests] = useState(false);
  const [selectedPost, setSelectedPost] = useState(null);
  const [isDrawerVisible, setIsDrawerVisible] = useState(isOpen);
  const [followedUserIds, setFollowedUserIds] = useState(new Set());
  const [followLoadingMap, setFollowLoadingMap] = useState({});

  useEffect(() => {
    setIsDrawerVisible(isOpen);
    if (isOpen) {
      const effectiveUserId = currentUserId || signedUser?._id;
      if (effectiveUserId) {
        dispatch(getFollowingAsync(effectiveUserId));
      }
      dispatch(getFollowRequestsAsync());
      dispatch(getActivityAsync());
    } else {
      setViewingAllRequests(false);
    }
  }, [isOpen, dispatch, currentUserId, signedUser?._id]);

  // Synchronize followed user IDs
  useEffect(() => {
    const ids = new Set();
    if (Array.isArray(following)) {
      following.forEach((item) => {
        let fId = "";
        if (item?.following) {
          fId =
            typeof item.following === "object"
              ? item.following._id || item.following.id
              : item.following;
        }
        if (fId) ids.add(fId.toString());
      });
    }
    if (Array.isArray(signedUser?.following)) {
      signedUser.following.forEach((item) => {
        const fId =
          typeof item === "object" ? item?._id || item?.id : item;
        if (fId) ids.add(fId.toString());
      });
    }
    setFollowedUserIds(ids);
  }, [following, signedUser?.following]);

  // Group activity into time buckets: Today, This week, Earlier
  const groupedActivity = useMemo(() => {
    const now = Date.now();
    const oneDay = 24 * 60 * 60 * 1000;
    const oneWeek = 7 * oneDay;

    const today = [];
    const thisWeek = [];
    const earlier = [];

    activity.forEach((act) => {
      const time = new Date(act.createdAt).getTime();
      const diff = now - time;
      if (diff < oneDay) {
        today.push(act);
      } else if (diff < oneWeek) {
        thisWeek.push(act);
      } else {
        earlier.push(act);
      }
    });

    return { today, thisWeek, earlier };
  }, [activity]);

  if (!isOpen && !selectedPost) return null;

  const handleOpenPost = (post) => {
    if (!post) return;
    setSelectedPost(post);
    setIsDrawerVisible(false);
  };

  const handleClosePostModal = () => {
    setSelectedPost(null);
    onClose();
  };

  const handleAccept = async (followerId) => {
    await dispatch(acceptFollowRequestAsync(followerId));
    dispatch(getFollowRequestsAsync());
    dispatch(getActivityAsync());
  };

  const handleReject = async (followerId) => {
    await dispatch(rejectFollowRequestAsync(followerId));
    dispatch(getFollowRequestsAsync());
  };

  const handleToggleFollow = async (targetUserId) => {
    if (!targetUserId || followLoadingMap[targetUserId]) return;
    const targetStr = targetUserId.toString();
    const wasFollowing = followedUserIds.has(targetStr);

    // Optimistic toggle
    setFollowedUserIds((prev) => {
      const next = new Set(prev);
      if (wasFollowing) next.delete(targetStr);
      else next.add(targetStr);
      return next;
    });

    setFollowLoadingMap((prev) => ({ ...prev, [targetStr]: true }));
    try {
      const res = await dispatch(toggleFollowAsync(targetStr)).unwrap();
      if (res?.status === "accepted" || res?.isFollowing === true) {
        setFollowedUserIds((prev) => new Set(prev).add(targetStr));
      } else if (res?.status === "not-following" || res?.isFollowing === false) {
        setFollowedUserIds((prev) => {
          const next = new Set(prev);
          next.delete(targetStr);
          return next;
        });
      }
    } catch (err) {
      console.error("Failed to toggle follow:", err);
      // Revert on error
      setFollowedUserIds((prev) => {
        const next = new Set(prev);
        if (wasFollowing) next.add(targetStr);
        else next.delete(targetStr);
        return next;
      });
    } finally {
      setFollowLoadingMap((prev) => ({ ...prev, [targetStr]: false }));
    }
  };

  const renderActivityItem = (act) => {
    const actUser = act.user || {};
    const actUserId = (actUser._id || actUser.id || "").toString();
    const actUsername = actUser.username || actUser.name || "user";
    const myIdStr = (currentUserId || signedUser?._id || "").toString();
    const isSelf = myIdStr && actUserId === myIdStr;
    const isFollowing = followedUserIds.has(actUserId);
    const isFollowLoading = !!followLoadingMap[actUserId];
    const isPostRelated =
      act.type === "tagged_post" ||
      act.type === "liked_post" ||
      act.type === "commented_post";

    let messageText = act.text || "interacted with you.";
    if (act.type === "tagged_post") {
      messageText = "tagged you in a post.";
    } else if (act.type === "liked_post") {
      messageText = "liked your post.";
    } else if (act.type === "accepted_request") {
      messageText = "accepted your follow request.";
    } else if (act.type === "new_follower") {
      messageText = "started following you.";
    }

    return (
      <div
        key={act._id}
        className="flex items-center justify-between p-2 rounded-xl hover:bg-gray-50 transition-colors"
      >
        <div className="flex items-center space-x-3 flex-1 min-w-0 pr-2">
          <Link
            to={`/profile/${actUserId}`}
            onClick={onClose}
            className="flex-shrink-0"
          >
            <Avatar
              src={actUser.profilePic}
              alt={actUsername}
              gender={actUser.gender}
              username={actUsername}
              className="w-11 h-11 rounded-full object-cover border border-gray-200"
            />
          </Link>

          <div className="flex-1 min-w-0 text-xs leading-snug">
            <p className="text-gray-900 break-words">
              <Link
                to={`/profile/${actUserId}`}
                onClick={onClose}
                className="font-semibold hover:underline mr-1 text-gray-900"
              >
                {actUsername}
              </Link>
              {isPostRelated && act.post ? (
                <span
                  onClick={() => handleOpenPost(act.post)}
                  className="text-gray-800 hover:underline cursor-pointer"
                >
                  {messageText}
                </span>
              ) : (
                <span className="text-gray-800">{messageText}</span>
              )}{" "}
              <span className="text-gray-400 text-[11px] whitespace-nowrap">
                {formatTimeAgo(act.createdAt)}
              </span>
            </p>
          </div>
        </div>

        {/* Right Action: Post Media Preview or Follow Button */}
        {isPostRelated && act.post?.media ? (
          <button
            type="button"
            onClick={() => handleOpenPost(act.post)}
            className="w-11 h-11 rounded-lg overflow-hidden flex-shrink-0 border border-gray-200 hover:opacity-85 transition cursor-pointer"
            aria-label="View post details"
          >
            {isVideoMedia(act.post.media, act.post.mediaType) ? (
              <video
                src={act.post.media}
                className="w-full h-full object-cover"
              />
            ) : (
              <img
                src={act.post.media}
                alt="Post thumbnail"
                className="w-full h-full object-cover"
              />
            )}
          </button>
        ) : (
          !isSelf &&
          actUserId && (
            <button
              type="button"
              disabled={isFollowLoading}
              onClick={() => handleToggleFollow(actUserId)}
              className={`text-xs font-semibold px-3.5 py-1.5 rounded-lg transition active:scale-95 flex-shrink-0 cursor-pointer ${
                isFollowing
                  ? "bg-[#EFEFEF] hover:bg-[#DBDBDB] text-gray-900"
                  : "bg-[#0095F6] hover:bg-[#1877F2] text-white"
              }`}
            >
              {isFollowLoading
                ? "..."
                : isFollowing
                ? "Following"
                : "Follow"}
            </button>
          )
        )}
      </div>
    );
  };

  return (
    <>
      {isDrawerVisible && (
        <div
          className="fixed inset-0 z-[60] bg-black/40 flex justify-start backdrop-blur-xs animate-in fade-in duration-200"
          onClick={onClose}
        >
          <div
            className="w-full sm:w-[410px] bg-white h-full shadow-2xl border-r border-gray-200 flex flex-col justify-between select-none animate-in slide-in-from-left duration-300 md:rounded-r-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
          {/* Header */}
          <div className="px-6 pt-6 pb-4 border-b border-gray-100 flex items-center justify-between">
            {viewingAllRequests ? (
              <div className="flex items-center space-x-3">
                <button
                  onClick={() => setViewingAllRequests(false)}
                  className="text-gray-700 hover:text-black p-1 -ml-2 text-2xl"
                  aria-label="Back to notifications"
                >
                  <IoChevronBack />
                </button>
                <h2 className="text-2xl font-bold text-gray-900">Follow requests</h2>
              </div>
            ) : (
              <h2 className="text-2xl font-bold text-gray-900 tracking-tight">
                Notifications
              </h2>
            )}

            <button
              onClick={onClose}
              className="text-gray-500 hover:text-black p-1 text-2xl rounded-full hover:bg-gray-100 transition"
              aria-label="Close notifications"
            >
              <IoClose />
            </button>
          </div>

          {/* Scrollable Content */}
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6 scrollbar-none">
            {/* Sub-view: Detailed Follow Requests List */}
            {viewingAllRequests ? (
              <div>
                {requests.length === 0 ? (
                  <div className="py-12 text-center text-gray-400 text-xs">
                    No pending follow requests.
                  </div>
                ) : (
                  <div className="space-y-2">
                    {requests.map((item) => {
                      const follower = item.follower || item;
                      const followerId = follower._id || follower.id || item._id;
                      const username = follower.username || follower.name || "user";
                      return (
                        <div
                          key={item._id || followerId}
                          className="flex items-center justify-between p-2.5 rounded-xl hover:bg-gray-50 transition"
                        >
                          <Link
                            to={`/profile/${followerId || ""}`}
                            onClick={onClose}
                            className="flex items-center space-x-3.5 flex-1 min-w-0 mr-2"
                          >
                            <Avatar
                              src={follower.profilePic}
                              alt={username}
                              gender={follower.gender}
                              username={username}
                              className="w-11 h-11 rounded-full object-cover border border-gray-200 flex-shrink-0"
                            />
                            <div className="truncate leading-tight">
                              <p className="font-semibold text-sm text-gray-900 truncate">
                                {username}
                              </p>
                              <p className="text-xs text-gray-400 truncate mt-0.5">
                                {follower.name || "requested to follow you"} · {formatTimeAgo(item.createdAt)}
                              </p>
                            </div>
                          </Link>

                          <div className="flex items-center space-x-2 flex-shrink-0">
                            <button
                              onClick={() => handleAccept(followerId)}
                              className="bg-[#0095F6] hover:bg-[#1877F2] text-white text-xs font-semibold px-4 py-1.5 rounded-lg transition shadow-sm"
                            >
                              Confirm
                            </button>
                            <button
                              onClick={() => handleReject(followerId)}
                              className="bg-[#EFEFEF] hover:bg-[#DBDBDB] text-gray-900 text-xs font-semibold px-4 py-1.5 rounded-lg transition"
                            >
                              Delete
                            </button>
                          </div>
                        </div>
                      );
                    })}
                  </div>
                )}
              </div>
            ) : (
              /* Main View: Grouped Sections */
              <>
                {/* Follow Requests Summary Row */}
                {requests.length > 0 && (
                  <div
                    onClick={() => setViewingAllRequests(true)}
                    className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 cursor-pointer transition border border-gray-100 shadow-xs"
                  >
                    <div className="flex items-center space-x-3.5">
                      <div className="relative flex -space-x-4">
                        {requests.slice(0, 2).map((item, idx) => {
                          const follower = item.follower || item;
                          return (
                            <Avatar
                              key={item._id || idx}
                              src={follower.profilePic}
                              alt="Requester"
                              gender={follower.gender}
                              username={follower.username || follower.name}
                              className="w-10 h-10 rounded-full object-cover border-2 border-white"
                            />
                          );
                        })}
                      </div>
                      <div>
                        <p className="font-semibold text-sm text-gray-900">
                          Follow requests
                        </p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          Approve or ignore requests
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center space-x-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-[#0095F6]" />
                      <span className="text-xs font-semibold text-[#0095F6]">
                        {requests.length}
                      </span>
                      <FaChevronRight className="text-xs text-gray-400 ml-1" />
                    </div>
                  </div>
                )}

                {/* Empty State */}
                {activity.length === 0 && requests.length === 0 && !loading && (
                  <div className="p-8 bg-gray-50 rounded-2xl text-center my-6">
                    <p className="text-sm text-gray-600 font-semibold mb-1">
                      No notifications yet
                    </p>
                    <p className="text-xs text-gray-400">
                      When people tag you in posts, like or comment on your photos, or follow you, you&apos;ll see it here.
                    </p>
                  </div>
                )}

                {/* Today's Activity */}
                {groupedActivity.today.length > 0 && (
                  <div className="space-y-2">
                    <span className="font-bold text-sm text-gray-900 block px-1">
                      Today
                    </span>
                    <div className="space-y-1">
                      {groupedActivity.today.map(renderActivityItem)}
                    </div>
                  </div>
                )}

                {/* This Week's Activity */}
                {groupedActivity.thisWeek.length > 0 && (
                  <div className="space-y-2">
                    <span className="font-bold text-sm text-gray-900 block px-1">
                      This week
                    </span>
                    <div className="space-y-1">
                      {groupedActivity.thisWeek.map(renderActivityItem)}
                    </div>
                  </div>
                )}

                {/* Earlier Activity */}
                {groupedActivity.earlier.length > 0 && (
                  <div className="space-y-2">
                    <span className="font-bold text-sm text-gray-900 block px-1">
                      Earlier
                    </span>
                    <div className="space-y-1">
                      {groupedActivity.earlier.map(renderActivityItem)}
                    </div>
                  </div>
                )}
              </>
            )}
          </div>
        </div>
      </div>
      )}

      {/* Full Post Details Modal on notification click */}
      {selectedPost && (
        <PostDetailsModal
          post={selectedPost}
          isOpen={true}
          onClose={handleClosePostModal}
        />
      )}
    </>
  );
}

export default NotificationsDrawer;
