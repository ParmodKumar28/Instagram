...
export function LeftSidebar() {
  ...
  const navItems = [
    {
      ...
      active: currentPath === "/",
      outlineIcon: (
        <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" className="flex-shrink-0 text-black">
          <path d="M3 9.5L12 2.5l9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" />
          <polyline points="9 22 9 12 15 12 15 22" />
        </svg>
      ),
      filledIcon: (
        <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor" className="flex-shrink-0 text-black">
    ...
    },
    {
      ...
      filledIcon: (
        <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="3.2" className="flex-shrink-0 text-black">
    ...
    },
    {
      ...
      outlineIcon: (
        <div className="w-6 h-6 rounded-full p-[1px] flex-shrink-0">
          <img
      ...
      filledIcon: (
        <div className="w-6 h-6 rounded-full p-[1px] flex-shrink-0 ring-2 ring-black ring-offset-1">
          <img
            src={currentUser?.profilePic || "https://placekitten.com/100/100"}
            alt="Profile"
            className="w-full h-full rounded-full object-cover"
  ...
  return (
    <aside
      ...
      className={`fixed left-0 top-0 h-screen bg-transparent flex flex-col justify-between py-6 z-50 select-none transition-all duration-300 ease-in-out ${
        isHovered ? "w-[245px] px-3" : "w-[72px] px-2"
      }`}
    >
      ...
      <div className="flex flex-col w-full">
        ...
        <Link
          to="/"
          className={`flex items-center mb-7 p-3 rounded-xl hover:bg-[#F2F2F2] transition duration-150 text-black ${
            isHovered ? "justify-start space-x-4" : "justify-center"
          }`}
          aria-label="Instagram Home"
        >
          <svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" strokeWidth="2" className="flex-shrink-0 text-black">
        ...
        <nav className="flex flex-col space-y-1.5 w-full">
          {navItems.map((item) => (
            <Link
              key={item.label}
              to={item.to}
              className={`group flex items-center p-3 rounded-xl transition-all duration-150 text-black hover:bg-[#F2F2F2] ${
                isHovered ? "justify-start space-x-4" : "justify-center"
            ...
            >
              ...
              {isHovered && (
                <span
                  className={`text-sm tracking-tight whitespace-nowrap transition-opacity duration-200 ${
      ...
      <div className="relative flex flex-col space-y-1.5 w-full">
        {/* More Popup Menu */}
        {showMoreMenu && (
          <div className="absolute bottom-16 left-1 w-56 bg-white border border-gray-200 rounded-2xl shadow-dropdown py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
            <Link
              to="/edit-profile"
              onClick={() => setShowMoreMenu(false)}
              className="block px-4 py-2.5 text-sm text-gray-800 hover:bg-gray-50 transition"
            >
              Settings
            </Link>
        ...
        <button
          ...
          className={`group flex items-center p-3 rounded-xl text-black hover:bg-[#F2F2F2] transition duration-150 focus:outline-none w-full ${
            isHovered ? "justify-start space-x-4" : "justify-center"
          }`}
          title={!isHovered ? "More" : undefined}
        >
          <div className="transform group-hover:scale-105 transition-transform duration-150 text-black">
            <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2.5" className="flex-shrink-0">
